...
- **Direct inversion of \( \mathbf{X}' \mathbf{X} + \lambda \bI \) (size \( p \times p \)) is \( O(p^3) \)**.
...
Bhattacharya et al. (2016) propose an **exact sampling algorithm** that avoids direct matrix inversion by leveraging:
1. **Latent variable augmentation** to restructure the sampling problem.
2. **Sherman–Morrison–Woodbury identity** to reduce matrix inversion complexity from \( O(p^3) \) to \( O(n^3) \).
3. **Block Gibbs sampling**, allowing efficient updates of the regression coefficients.
...
- \( \mathbf{y} \) is the \( n \)-dimensional response vector.
- \( \mathbf{e} \) represents independent Gaussian noise.
...
where:
- \( \lambda_j \) are **local shrinkage parameters**, controlling the sparsity of each coefficient.
- \( \tau \) is a **global shrinkage parameter**, controlling overall sparsity.
- Different choices of \( p(\lambda_j) \) lead to different priors (e.g., Horseshoe, Bayesian Lasso).
...
1. Sample \( \mathbf{u} \sim \mathcal{N}(\mathbf{0}, \Lambda) \).
2. Sample \( \boldsymbol{\delta} \sim \mathcal{N}(\mathbf{0}, \mathbf{I}_n) \).
