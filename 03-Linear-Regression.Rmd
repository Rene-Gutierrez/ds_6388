...
\[
p(\beta_j | \sigma^2) = \frac{\lambda}{2\sigma} \exp \left( - \frac{\lambda}{2\sigma} | \beta_j | \right).
\]
...
\[
p(\tau_j^2 | \sigma^2) = \frac{\lambda^2}{2} \exp \left( -\frac{\lambda^2}{2} \tau_j^2 \right).
\]
...
\[
\hat{\boldsymbol{\beta}} = \arg \min_{\boldsymbol{\beta}} \left\{ \frac{1}{2\sigma^2} \|\mathbf{y} - \mathbf{X} \boldsymbol{\beta} \|_2^2 + \frac{\lambda}{2\sigma} \|\boldsymbol{\beta}\|_1 \right\}.
\]

This **exactly recovers the traditional Lasso estimator**, where the regularization term depends on \( \lambda / \sigma \).  
...
- **As \( \sigma^2 \to 0 \)**: The likelihood becomes narrow, making the Lasso penalty more influential, leading to **greater shrinkage**.  
- **As \( \sigma^2 \to \infty \)**: The likelihood dominates, reducing the influence of the prior and making the solution behave more like **OLS**.

---

### **Key Differences from Ridge Regression**  

Unlike Bayesian Ridge regression, where \( \boldsymbol{\beta} \) follows a **Gaussian prior**, the **Laplace prior** in Bayesian Lasso results in **sparser solutions** by pushing many coefficients to **exactly zero**. This makes Bayesian Lasso particularly useful for:  

- **Feature selection** in high-dimensional settings.  
- **Interpretable models**, where only the most relevant predictors remain.  
- **Handling multicollinearity**, since irrelevant predictors shrink to zero rather than being retained with small weights.  

Would you like me to extend this discussion to **Bayesian computation methods (e.g., Gibbs sampling) for the Bayesian Lasso**?
